...
let handler = async(m, { conn, usedPrefix, command, text, participants }) => {
  if(command === "jadian") {
    ...
    do b = ps.getRandom()
    while(b === a)
    const caption = `*Love Message...* ${toM(a)} 鉂わ笍 ${toM(b)}\n${ktnmbk.getRandom()}`
  ...
  if(!text && !m.quoted) return await m.reply("Reply atau tag orangnya!")

  const user = m.quoted?.sender || text.replace(/[^0-9]/g, "") + "@s.whatsapp.net"
  if(!(await conn.onWhatsApp(user))[0]) return await m.reply("Nomor tidak terdaftar di WhatsApp!")
  if(!db.data.users[user]) return await m.reply("Nomor tidak terdaftar di database bot!")
  if(!participants.find(v => v.id === user)) return await m.reply("Gagal! dia harus ada di grup ini")
  try {
    if(user === m.sender) return await m.reply("Tidak bisa berpacaran dengan diri sendiri.")
    if(user === conn.user.jid) return await m.reply("Maaf, saya adalah bot. Anda tidak dapat berpacaran dengan saya.")

    const pacar = db.data.users[user].pasangan
    const spac = db.data.users[m.sender].pasangan
    if(db.data.users[m.sender].pasangan && db.data.users[db.data.users[m.sender].pasangan].pasangan == m.sender && db.data.users[m.sender].pasangan != user) {
      await m.reply(`Kamu sudah berpacaran dengan @${db.data.users[m.sender].pasangan.split("@")[0]}\n\nSilahkan putus dulu (ketik .putus untuk memutuskan hubungan) untuk menembak @${user.split("@")[0]}\n\nBtw yang setia dikit banget!`, null, {
        mentions: [user, db.data.users[m.sender].pasangan]
      })
    } else if(db.data.users[user].pasangan != "") {
      if(pacar) {
        if(m.sender == pacar && db.data.users[m.sender].pasangan == user) return await m.reply(`Anda sudah berpacaran dengan ${spac.split("@")[0]}`, null, {
          mentions: [spac]
        })
        await m.reply(`Maaf, @${user.split("@")[0]} sudah berpacaran dengan @${pacar.split("@")[0]}\nSilahkan cari pasangan lain!`, null, {
          mentions: [user, pacar]
        })
      } else {
        db.data.users[m.sender].pasangan = user
        await m.reply(`${ktnmbk.getRandom()}\n\nAnda baru saja mengajak @${user.split("@")[0]} berpacaran\n\nSilahkan menunggu jawaban darinya!\n\nKetik *${usedPrefix}terima @user* untuk menerima\n*${usedPrefix}tolak @user untuk menolak*`, null , {
          mentions: [user]
        })
      }
    } else if (db.data.users[user].pasangan == m.sender) {
      db.data.users[m.sender].pasangan = user
      await m.reply(`Selamat anda resmi berpacaran dengan @${user.split("@")[0]}\n\nSemoga langgeng dan bahagia selalu 馃コ馃コ馃コ`, null, {
        mentions: [user]
      })
    } else {
      db.data.users[m.sender].pasangan = user
    ...
    await m.reply("Error!\n\nApa yang terjadi? ada error/kesalahan dalam kode ini\nApa yang harus dilakukan? kalau kamu developer bot ini, silahkan cek log bot. Kalau bukan, laporkan ke developer/owner bot ini")
    console.error(e)
  }
}
