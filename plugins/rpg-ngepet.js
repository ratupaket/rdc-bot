...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
  try {
    db.data.users[m.sender].lastngepet = db.data.users[m.sender].lastngepet || 0
    let randomaku = `${Math.floor(Math.random() * 0 - 149)}`.trim()
    let randomkamu = `${Math.floor(Math.random() * 0 - 19)}`.trim() // Increased chances of failure
    let Aku = (randomaku * 1)
    ...
    let botol = global.pauthor
    
    let __timers = (new Date - db.data.users[m.sender].lastngepet)
    let _timers = (18000000 - __timers) 
    let timers = clockString(_timers)
    let user = db.data.users[m.sender]
    if (new Date - db.data.users[m.sender].lastngepet > 18000000) { // Changed to 5 hours
      if (Aku > Kamu) {
        ...
        db.data.users[m.sender].lastngepet = new Date * 1
      } else if (Aku < Kamu) {
