...
let handler = async (m, { conn, usedPrefix, command }) => {
        try {
                //let res = await fetch(`https://api.lolhuman.xyz/api/storynime?apikey=jjevingeo`)
               // let fimg = await fetch(res.url)
                let fimgb = Buffer.from(await fimg.arrayBuffer())
                if (Buffer.byteLength(fimgb) < 100000) throw new e()
               // conn.sendButton(m.chat, `_Random Story Anime_`, pauthor, fimgb, [[`Next`, `${usedPrefix + command}`]], m)
        } catch (e) {
                console.log(e)
                try {
                        conn.sendButton(m.chat, `_ezzzz_`, pauthor, dir[Math.floor(Math.random() * dir.length)], [[`Next`, `${usedPrefix + command}`]], m)
                } catch (e) {
                        console.log(e)
