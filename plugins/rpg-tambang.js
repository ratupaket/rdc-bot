...
function formatTime(ms) {
  return ["\n" + (isNaN(ms) ? "--" : Math.floor(ms / 864e5)), " *Hari*\n", isNaN(ms) ? "--" : Math.floor(ms / 36e5) % 24, " *Jam*\n", isNaN(ms) ? "--" : Math.floor(ms / 6e4) % 60, " *Menit*\n", isNaN(ms) ? "--" : Math.floor(ms / 1e3) % 60, " *Detik*"].map(v => v.toString().padStart(2, 0)).join("");
}
async function handler(m, {
  conn,
  text
}) {
  db.data.tambang = db.data.tambang || {};
  let user = db.data.users[m.sender];
  if (m.sender in db.data.tambang) return db.data.tambang[m.sender].currentArea >= db.data.tambang[m.sender].areas.length ? m.reply("🏆 Anda telah menyelesaikan semua area pertambangan.") : m.reply("⏳ Anda masih memiliki area pertambangan yang belum selesai. Silakan selesaikan terlebih dahulu.");
  if ("start" === text) {
...
handler.before = async m => {
  if (db.data.tambang = db.data.tambang || {}, !(m.sender in db.data.tambang)) return;
  ...
  if ("stop" === msg) return m.reply("❌ Pertambangan telah dihentikan. Ketik *'tambang start'* untuk memulai pertambangan kembali."),
    delete db.data.tambang[m.sender], !1;
  if (currentArea < areas.length && areas[currentArea].txt === msg) {
    let {
      area,
      reward
    } = areas[currentArea];
    if (user.exp += reward.exp, user.crystal += reward.crystal, hasilTambang++, totalReward += reward.exp + reward.crystal, currentArea++, db.data.tambang[m.sender].currentArea = currentArea, db.data.tambang[m.sender].hasilTambang = hasilTambang, db.data.tambang[m.sender].totalReward = totalReward, db.data.tambang[m.sender].lastTambangTime = Date.now(), currentArea >= areas.length) return m.reply(`🎉 Selamat! Anda telah menyelesaikan semua area pertambangan.\nTotal hasil tambang: ${hasilTambang}\nExp yang didapatkan: ${totalReward}\nTotal crystal yang didapatkan: ${totalReward}`),
      delete db.data.tambang[m.sender], !1;
    {
