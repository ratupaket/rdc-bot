...
let handler = async (m, {
  ...
  if (command == 'tiktokslide' || command == 'ttslide') {
    ...
    try {
      const api = await fetch(`https://api.botcahx.eu.org/api/download/tiktokslide?url=${text}&apikey=rdculous`);
  ...
  if (command == 'douyinslide' || command == 'douyinfoto') { 
    ...
    try {
      const api = await fetch(`https://api.botcahx.eu.org/api/download/douyinslide?url=${text}&apikey=rdculous`);
      const res = await api.json();
      for (let i of res.result.images) {
        await sleep(3000);
        conn.sendMessage(m.chat,{ image :{ url : i } , caption : `*Title*: ${res.result.title}` }, { quoted: m });         
