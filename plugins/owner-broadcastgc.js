...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	let chats = Object.entries(Connection.store.chats).filter(([jid, chat]) => jid.endsWith('@g.us') && chat.isChats && !chat.metadata?.read_only && !chat.metadata?.announce)
	...
	let teks = command.includes('meme') ? `${text}\n\n*[ Broadcoast Group ]*` : `*[ Broadcast Group ]*\n\n${text}`
	for (let [id, _] of chats) {
		try {
        const { participants } = _

			if (/image|video/g.test(mime)) await conn.sendFile(id, img, '', teks, null, false, { mentions: participants.map(a => a.id) })
