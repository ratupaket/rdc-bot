let handler = async (m, { conn, text, participants }) => {
	...
	if (/video/g.test(mime)) {
		...
		conn.sendMessage(m.chat, { video: media, caption: teks, mentions: participants.map(a => a.id) }, )
	} else if (/image/g.test(mime)) {
		let media = await q.download?.()
		conn.sendMessage(m.chat, { image: media, caption: teks, mentions: participants.map(a => a.id) }, )
	} else {
