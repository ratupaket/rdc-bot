...
let handler = async (m, { args, conn, usedPrefix, command, isPrems, isAdmin, isOwner, participants }) => {
	...
	if (isPrems || isAdmin || isOwner) {
		...
		chat.spamcount = 0
		await conn.reply(grup, `Group di *mute* selama ${total} menit.`, )
	} else throw `khusus admin`
}
