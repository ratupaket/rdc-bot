...
let handler = async (m, { command, usedPrefix, args, isPrems }) => {
	...
	if (!listItems[item] && somematch(['buy','shop','beli'], command.toLowerCase())) return m.reply(info.replace('*', '*'))
	if (!listItems[item] && somematch(['sell','jual'], command.toLowerCase())) return m.reply(infos.replace('%', '```'))
	let paymentMethod = Object.keys(listItems[item]).find(v => v in user)
	if (somematch(['buy','shop','beli'], command.toLowerCase())) {
		if (isPrems && item == 'limit') throw `[!] Premium User tidak perlu limit.`
		if (somematch(['horse', 'cat', 'fox', 'dog', 'wolf', 'centaur', 'phoenix', 'dragon', 'rumahsakit', 'restoran', 'pabrik', 'tambang', 'pancingan', 'umpan', 'pelabuhan'], args[0].toLowerCase())) {
			if (user[`${item}`] == 0) {
