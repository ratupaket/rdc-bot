...
let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  if (new Date - db.data.users[m.sender].lastmulung< 1800000) throw `Clan kamu sudah kecapekan berperang\n\nTunggu ${msToTime(time - new Date())} lagi`
  ...
  let kesempatan = []
  for (i=0;i<db.data.users[m.sender].money;i++) kesempatan.push(m.sender)
  for (i=0;i<db.data.users[lawan].money;i++) kesempatan.push(lawan)
