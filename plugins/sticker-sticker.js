...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	...
	if ((text || '-').match(/^(https?:\/\/)?(www\.)?([a-zA-Z0-9]+(-?[a-zA-Z0-9])*\.)+[\w]{2,}(\/\S*)?$/)) {
		let s = await sticker(false, text, pauthor)
		...
		if (/image|video|viewOnce/g.test(mime)) {
			...
			try {
				buffer = await (ch > 1 ? addExif(await video2webp(img, isNaN(fps) ? 15 : fps), packname, author) : ch > 0 ? sticker(img, false, packname, author) : /webp/g.test(mime) ? addExif(img, packname, author) : createSticker(img, { pack: pauthor, type: StickerTypes.FULL }))
