...
export async function handler(chatUpdate) {
	...
	try {
		...
		try {
			...
			if (m.sender.endsWith('@s.whatsapp.net')) {
				...
				if (user) {
					...
					if (!isNumber(user.udang))
						user.udang = 0
                    if (!isNumber(user.attack))
                        user.attack = 0
                    if (!isNumber(user.strenght)) 
                        user.strenght = 0
                    if (!isNumber(user.speed))
                        user.speed = 0
                    if (!isNumber(user.defense))
                        user.defense = 0
                    if (!isNumber(user.stamina))
                        user.stamina = 100
                    if (!isNumber(user.regeneration)) 
                        user.regeneration = 0                    
                    if (!isNumber(user.dana))
                        user.dana = 0
                    if (!isNumber(user.gopay))
                        user.gopay = 0
                    if (!isNumber(user.ovo)) 
                        user.ovo = 0
			...
			if (datas) {
				if (!('packname' in datas)) datas.packname = 'WhatsApp Bot By'
				if (!('author' in datas)) datas.author = '© Rdculous'
				if (!('namebot' in datas)) datas.namebot = ''
		...
		const isOwner = isROwner || db.data.datas.owner.map(([number]) => number).map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
		const isPrems = isOwner || db.data.datas.prems.map(v => v.user).includes(m.sender)
		const isBotOwner = global?.conns?.some(v => v.owner === m.sender)
            ...
            case 'remove':
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        let pp = 'https://telegra.ph/file/0615e1831e71cb13e3f50.png'
                        try {
                            pp = await this.profilePictureUrl(user, 'image')
                        } catch (e) {} finally {
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc ? String.fromCharCode(8206).repeat(4001) + groupMetadata.desc : '') :
                                (chat.sBye || this.bye || conn.bye || 'Bye, @user!')).replace('@user', '@' + user.split('@')[0])
                                let wel = 'https://ibb.co.com/QkMmP6n'  //https://telegra.ph/file/96c857aa540aef7d385eb.jpg
                                let lea = 'https://ibb.co.com/H2W9XT2'  //https://telegra.ph/file/999b3af6dac1b48769ee6.jpg
 ...
 * @this {import('./lib/connection').Socket}
 */
export async function onCall(call) {
  if(!(call.content[0]?.tag == "offer")) return

  const callId = call.content[0].attrs["call-id"]
  const callCreator = call.content[0].attrs["call-creator"]

  await this.rejectCall(callId, callCreator)
}
...
global.dfail = (type, m, conn) => {
	let msg = {
		...
		ownerBot: "Kamu harus memiliki bot yang dibuat di dalam bot ini!",
		mods: `Perintah ini hanya dapat digunakan oleh Owner`,
		premium: `Perintah ini hanya dapat digunakan oleh User Premium`,
		group: `Perintah ini hanya dapat digunakan di grup`,
		private: `Perintah ini hanya dapat digunakan di Chat Pribadi`,
		admin: `Perintah ini hanya untuk Admin grup!`,
		nsfw: `NSFW di Nonaktifkan`,
