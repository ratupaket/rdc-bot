...
`.trim()
		...
		reply: {
			...
			value(jid, text = '', quoted, options = {}) {
				return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, { mention: conn.parseMention(text), ...options }) : conn.sendMessage(jid, { mentions: conn.parseMention(text), ...options, text }, { quoted, ...options, ...ephemeral })
			},
			writable: true,
		},
		preSudo: {
...
export function protoType() {
	...
	/**
	 * Untuk nyembunyiin keys dari for in loop
	 * @param {*} proto - Object nya
	 * @param {String} keys - Keys nya
	 */
	const hide = (proto, keys) => void Object.defineProperty(global[proto.name || proto].prototype, keys, { enumerable: false })
    hide(Array, "getRandom")
	hide(Array, "getFileType")
}
