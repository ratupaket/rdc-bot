...
const killww = (from, id, data) => {
  ...
  for (let j = 0; j < room.dead.length; j++) {
    let idd = getPlayerById(from, room.player[0].id, room.dead[j], data);
    if (!idd) return false;
...
async function win(x, t, conn, data) {
  ...
  if (getWinner(x.room, data).status === false || x.iswin === false) {
    ...
    textnya += "Masing-masing mendapatkan *Rp 10.000.000*\n\n"
    // eslint-disable-next-line no-undef
    for(let user of [...new Set(ment)]) db.data.users[user].money += 10_000_000
    return await conn
      .sendMessage(sesinya, {
        text: textnya,
        contextInfo: {
    ...
    textnya += "Masing-masing mendapatkan *Rp 7.000.000*\n\n"
    // eslint-disable-next-line no-undef
    for(let user of [...new Set(ment)]) db.data.users[user].money += 7_000_000
